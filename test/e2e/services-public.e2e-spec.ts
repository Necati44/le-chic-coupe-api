...
describe('Services (PUBLIC) E2E', () => {
  ...
  beforeAll(async () => {
    ...
    const servicesStub = {
      findAll: jest.fn().mockResolvedValue({ items: [{ id: 's1', name: 'Coupe' }], total: 1, skip: 0, take: 10 }),
  ...
  it('GET /services → 200', async () => {
    ...
    expect(res.body).toEqual({
      ...
      total: 1, skip: 0, take: 10,
    });
  });

  it('GET /services/:id → 200', async () => {
    const res = await request(app.getHttpServer()).get('/services/s1').expect(200);
