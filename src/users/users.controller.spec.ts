...
describe('UsersController (unit)', () => {
  ...
  it('finalize: retourne needsProfile=false si le profil existe déjà', async () => {
    usersService.findByFirebaseUid.mockResolvedValueOnce({ id: 'u1', email: 'e@x' } as any);
    ...
    expect(res).toEqual({ needsProfile: false, user: { id: 'u1', email: 'e@x' } });
  ...
  it('finalize: crée le profil si inexistant', async () => {
    ...
    usersService.createFromFirebase.mockResolvedValueOnce({ id: 'u2', email: 'e@x' } as any);
    ...
    expect(usersService.createFromFirebase).toHaveBeenCalledWith('uid-2', 'e@x', dto);
    expect(res).toEqual({ needsProfile: false, user: { id: 'u2', email: 'e@x' } });
  });
});
